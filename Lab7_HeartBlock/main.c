...
int main(void){
  ...
  while(1){          // Follows the nine steps list above
   ...
   Delay1ms(25); // f) wait 250ms
   SetVT(); // g) VT signal goes high
   Delay1ms(25); // h) wait 250ms
...
void WaitForASLow(void){
// write this function
	do {sw1=GPIO_PORTF_DATA_R&0x10 ; //read pf4 sw1 
	}
		while (sw1!=0x00); //wait for pf4 to be low
}
...
// write this function
	do {sw1=GPIO_PORTF_DATA_R&0x10 ; //read pf4 sw1 
		...
		while (sw1!=0x10);  //wait for pf4 to be high
}
...
void SetVT(void){
// write this function
	GPIO_PORTF_DATA_R=GPIO_PORTF_DATA_R|0x02;    //set pf1 to 1 the red led is on
}
...
void ClearVT(void){
// write this function
	GPIO_PORTF_DATA_R=GPIO_PORTF_DATA_R&(~0x02); //set pf1 to 0 the red led is off
}
...
void SetReady(void){
// write this function
	GPIO_PORTF_DATA_R=GPIO_PORTF_DATA_R|0x08;  //set pf3 to 1 the red led is on
}
...
void ClearReady(void){
// write this function
	GPIO_PORTF_DATA_R=GPIO_PORTF_DATA_R&(~0x08);  //set pf3 to 0 the red led is off
}
...
// write this function
unsigned long count;
  while(msec > 0 ) { // repeat while still 10msec to delay
    count = 153860;
    while (count > 0) {
      count--;
